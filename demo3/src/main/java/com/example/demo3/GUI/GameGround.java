...
public class GameGround extends Application {
    ...
    public void start(Stage stage) throws IOException, SQLException {
        ...
        stage.getScene().getWindow().setOnCloseRequest(windowEvent -> {
            ...
            // update player
            Player.playing.setScore(score + Player.playing.getScore());
...
//            int speed = rs.getInt("speed");
    ...
    private ProgressIndicator createProgressIndicator(Hero hero) {
        ...
        pi.setTranslateX(hero.getTranslateX());
        pi.setTranslateY(hero.getTranslateY() - 10);
//        pi.setMaxWidth(50);
//        pi.setMaxHeight(25);
                ...
                all.stream().filter(h1::isEnemy).forEach(enemy -> {
                    ...
                    if (h1.detectCollision(enemy)) {
                        ...
                        new Thread(() -> {
                            ...
                            while (h1.isAlive() && enemy.isAlive() && continueGame) {
    ...
    public void dragHeroes() {
        ...
        studentHeroes = new Group(s1, s2, s3, s4);
        bg.getChildren().add(studentHeroes);

        for (Node h : studentHeroes.getChildren()) {
